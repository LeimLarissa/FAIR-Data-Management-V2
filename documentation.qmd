...
[^1]: „metadata“. Merriam-Webster Dictionary, accessed 7. July 2024, <https://www.merriam-webster.com/dictionary/metadata>.
...
1.  **Related publication**: Most published datasets are accompanied by scientific papers that describe in detail what was done to create the dataset, what it consists of, and how it was analysed. While this can provide very detailed information, it makes it difficult to evaluate the dataset on its own. Ideally, you should provide enough metadata separate from your paper to allow others to assess whether they could reuse it.

2.  **Repository description**: You will often be asked to provide metadata when you upload your dataset to a repository. The quality and amount of metadata you can provide varies from repository to repository. We covered repositories in Chapter 1), but remember that in most cases it is your responsibility to fill in the metadata fields as accurately and concisely as possible.

3.  **README file**: Even before you publish your dataset, you should keep track of your metadata. This will prevent you from losing track of it, and will help inform others. A good way to do this is to create a README file. We look at README files in the next section.
:::

::: callout-caution
## Dependencies, requirements, and virtual environments

While the metadata section is roughly transferable to software/code I think an important point missing from the documentation aspect for software specifically are dependencies, requirements or whole virtual environments. Maybe they could be introduced here with links to more cohesive explanations.

Does it a require an introduction to the different levels of sophistication? That way the selection on what method to use can be connected to the level of sophistication of the code itself.  

This section in the GWP for RSE document might be of interest: [https://gwp-rse-ff130b.pages.gwdg.de/en/faqs/reprod/](https://gwp-rse-ff130b.pages.gwdg.de/en/faqs/reprod/), but I don't see it completely covering the topic. 

Maybe this resource would be helpful? [https://book.the-turing-way.org/reproducible-research/renv/](https://book.the-turing-way.org/reproducible-research/renv/)

Or maybe this resource: https://s3-school.github.io/s3-2026-lectures/2.1-coding-environment-good-practices/2.1.1-coding-environment-IDE/
This one seems to be a bit more advanced than the turing way introduction. 
::: 
...
-   What do the data files contain?\
    -   Code package for the execution of an extensive comparison of model reduction methods
    -   From file structure: Python scripts and MATLAB scripts
-   When was the dataset generated?\
    -   From file structure: Only Version 1 published on February 13, 2019
    -   The dataset was published on February 13, 2019.
-   Where was the dataset generated?\
    -   Max Planck Institute for Dynamics of Complex Technical Systems, CAU Kiel
    -   From the related publication: Max Planck Institute for Dynamics of Complex Technical Systems, CAU Kiel
-   Why was the dataset generated?\
    -   For the structure preserving model reduction of second order mechanical systems exemplified on an artificial fish tail model
    -   From the related publication: More detailed description of course in publication
-   How was the dataset generated?\
    -   Python; MATLAB
    -   From related publication: Python; MATLAB
-   What research is this dataset connected to?\
    -   Not indicated on dataset level.
    -   <https://doi.org/10.1515/auto-2019-0027> (Not provided on dataset level)
-   Can this dataset be reused?\
    -   The dataset is under a GNU General Public License v2.0 only, which would allow re-use under the given conditions.
:::
...
As already mentioned this README structure and template does not work well for software/code especially with the additional component of the README being usually the "front page" and contact point for users on platforms like GitHub. Is there another template we can provide and should the task be adapted to fit the additonal template? 
...
-   4 sections can be filled based on the Example dataset README alone.
-   Not all fields are applicable for code datasets/scripts
-   ?? open fields can be filled based on the information collected in task 2.1.
-   Find the filled out README template for example 5 [here](/Example5_READMETemplate.md)
:::
...
-   No overview in the MATLAB code structure
-   Script usage is explained in the README-file.
:::

::: callout-caution 
## Software quality tools 
Would it make sense to introduce linters or formatting tools here in this section? Since codebooks are kind of helping make the data structures more readable. Linters and formatting tools could help with readability of code without putting in a lot of effort on skill development. 

There is are slides on the topic from the S3 School: [https://vuillaut.github.io/lectures/quality_tools/1](https://vuillaut.github.io/lectures/quality_tools/1)

Also if referring to software quality overall, this RSQkit might be a useful resource: [https://everse.software/RSQKit/](https://everse.software/RSQKit/)
:::
